import { BsBookmark, BsShareFill, BsGeoAlt } from "react-icons/bs";
...
const CardDetails = ({card}) => {
  ...
  // const info = card?.description.split('\n').map(x=>x.trim()).filter(Boolean);
  // const benefitsList = info[4].split('.').map(x=>x.trim()).filter(Boolean)
    ...
    <div className="container bg-white mx-auto px-4 pt-6 pb-6 lg:pt-14 lg:pb-40">
      <div className="lg:flex lg:gap-x-8 xl:gap-x-[134px] max-w-[1257px] m-auto">
        <div>
          ...
          <main>
            <div className="lg:flex shrink-0 justify-between items-start lg:pb-2">
              ...
              <div className="flex items-center justify-between pb-3.5 lg:p-0">
                <p className="lg:hidden postCreatedAt">{postDate}</p>
            ...
            <div>
              ...
              <ul>
                {benefitsList.map(item=><li className="list-disc text-lg text-[#3A4562]" key={item}>{item}</li>)}
              </ul>
            </div>
          ...
          <div>
            <h3 className="sectionTitle border-b-2 border-[#3A4562] pb-3 mb-4">Additional info</h3>
            <p className="text-[18px] tracking-[-0.5625px] pb-4">Employment type</p>
            <div className="flex gap-2 pb-6">
              <div className="flex gap-4 flex-row">
                {card.employment_type.map(e=><div className="w-[122px] h-[52px] bg-[#e8e8f7] flex items-center justify-center text-[#718ace] rounded-lg uppercase text-[14px] font-bold" key={e}>{e}</div>)}
              </div>
            </div>
            <p className="text-[18px] tracking-[-0.5625px] pb-4">Benefits</p>
            <div className="flex gap-2 pb-6">
              <div className="flex gap-4 flex-row pb-16">
                {card.benefits.map(e=><div className="h-[52px] w-[170px] md:w-[222px] bg-[#fff8d9] border-[#FFCF00] flex flex-row items-center justify-center text-[#867d49] rounded-lg font-bold uppercase text-[14px] " key={e}>{e}</div>)}
              </div>
            </div>
          </div>
          {/* ___________________________________________Additional info___________________________________ */}
        ...
        <div>
          <div className="w-full lg:w-[330px] xl:w-[402px]"> 
            <div className="bg-[#2A3047] border rounded-t-lg text-[#E8EBF3] py-8 h-[215px] w-full flex justify-center relative overflow-hidden">
              <div className="z-10">
                <p className="font-bold tracking-[0.23619px] leading-[1.18] text-[#E7EAF0] lg:text-[20px] leading-[1.25] tracking-[-0.625px] pb-2 lg:pb-4">
                  {card.name}
                </p>
                <div className="flex ">
                  <p className='text-base font-normal flex row-auto items-center gap-x-1 text-[#E7EAF0]'>
                    <BsGeoAlt/> {card?.address} 
                  </p>
                </div>
